...
module.exports = {
    ...
    submissionConfirmationTemplate: function (reference, addressBody, url, customerRef, serviceType) {
        ...
        if (customerRef !== "undefined" && customerRef !== null && customerRef !== "") {
            if (serviceType === 1) {//standard service
                body += ' <h1>Application confirmation</h1> ' +
                    ...
                    '<p>Please ensure your documents have been certified by the relevant authority with and original wet ink signature and/or seal (e.g solicitor, notary public, government official. medical practitioner, etc.).</p>' + '<br>' +
                ...
                body += ' <h1>Application confirmation</h1> ' +
                    ...
                    '<img src="' + url + '/qr-code-converter/' + reference + '" style="margin: 0 auto;display: block;">' +
                    '</td> </tr> </tbody></table>' + '<br >' +
                    '<p>Please ensure your documents have been certified by the relevant authority with and original wet ink signature and/or seal (e.g solicitor, notary public, government official. medical practitioner, etc.).</p>';
            }
        } else {
            if (serviceType === 1) { //standard service
                body += ' <h1>Application confirmation</h1> ' +
    ...
    accountExpiringTemplate: function (url, accountExpiryDate, dayAndMonth, email) {
        var body = '';

        body += ' <h1>Sign in if you want to keep the account you registered using <a style="color: black; text-decoration: none">' + email + '</a></h1> ' +
            '<p style="font-size: 14px">As you have not signed in to your <a style="color: black; text-decoration: none"><font style="display: none">@</font>' + email + '</a> legalisation online account for a year it will be deleted on ' + accountExpiryDate + '.</p>' +
    ...
    accountExpiredTemplate: function (url, email) {
        var body = '';

        body += ' <h1>Your <a style="color: black; text-decoration: none">' + email + '</a> account has been deleted</h1> ' +
            '<p style="font-size: 14px">As you have not signed in to your <a style="color: black; text-decoration: none"><font style="display: none">@</font>' + email + '</a> legalisation online account for over a year your account has now been deleted.</p>' +
