module.exports = (router, notify, notifySettings) => {
  ...
  router.post('/failed-documents', function failed_certs_string(req, res) {
    ...
    for (let failedCert = 0; failedCert < failed_certs.length; failedCert++) {
      failedCertList.push(failed_certs[failedCert].doc_title)
    }
